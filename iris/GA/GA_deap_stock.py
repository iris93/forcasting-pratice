...
creator.create("FitnessMax", base.Fitness, weights=(1.0,))   #create the type
creator.create("Individual", list, fitness=creator.FitnessMax)    #define individual

toolbox = base.Toolbox()    #The Toolbox is a container for tools of all sorts including initializers that can do what is needed of them.
...
import pickle
data_file = open('inputdata/train_data.pkl','rb')
# run cnn with stock data ====================start
train_data = pickle.load(data_file)
trX = train_data["trX"]
trY = train_data["trY"]
print "total train_length:",len(trX)
def evalOneMax(individual):
    error = 0
    for i in range(len(trX[0:100])):
        label = 0
        for j in range(len(individual)):
            label +=individual[j]+trX[i][j]
        error +=abs(label-trY[i])
    return -error,
...
# register the crossover operator,http://deap.readthedocs.io/en/master/api/tools.html#operators
toolbox.register("mate", tools.cxTwoPoint)

# register a mutation operator with a probability to
# flip each attribute/gene of 0.05,you can change the operator use in the mutate step.
toolbox.register("mutate", tools.mutFlipBit, indpb=0.05)

# operator for selecting individuals for breeding the next
# generation: each individual of the current generation
# is replaced by the 'fittest' (best) of three individuals(tournsize)
